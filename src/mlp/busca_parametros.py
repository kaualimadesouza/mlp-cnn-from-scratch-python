...
def avaliar(dataset: Dataset, taxa: float, neuronios: int) -> list[tuple]:
    ...
    for paciencia in PACIENCIAS:
        ...
        linhas.append((mse_no_corte, taxa, neuronios, paciencia, parada, acuracia))
    return linhas
...
def salvar_csv(resultados: list[tuple], caminho: str) -> None:
    ...
    with open(caminho, "w") as f:
        f.write(
            ...
            "epoca_parada,acuracia_teste\n"
        )
        for mse, taxa, n, paciencia, parada, acuracia in resultados:
            f.write(f"{mse:.6f},{taxa},{n},{paciencia},{parada},{acuracia:.4f}\n")
...
def main() -> None:
    ...
    for taxa in TAXAS:
        for n in NEURONIOS:
            ...
            mse, _, _, paciencia, parada, acuracia = min(linhas)
            print(
    ...
    mse, taxa, n, paciencia, parada, acuracia = resultados[0]
    print(
